...
export default function BuyerDetails({ data, errors, onChange }) {
    ...
    return (
        <Card sx={{ mb: 3 }}>
            <CardHeader title="Buyer Details" />
            <CardContent>
                <Box display="flex" flexDirection="column" gap={3}>
                    <TextField fullWidth label="Buyer Name" name="buyer_name" value={data?.buyer_name || ''} onChange={handleChange} variant="outlined" />
                    <TextField fullWidth label="Buyer Address" name="buyer_address" value={data?.buyer_address || ''} onChange={handleChange} variant="outlined" />
